...
#              other. Karatsuba is efficient than quadratic.
#
# Complexity: O(n^1.58), better than conventional algo.
# 
# Implementation: Both numbers can be expressed as high and low components like
#                 x = (10^n/2)*x_H + x_L and y = (10^n/2)*y_H + y_L
#                 where n is max of number of digits in x or y.
#                 So,
#                 x*y = (10^n)*x_H*y_H + (10^n/2)*(x_H*y_L + x_L*y_H) + x_L*y_L
#                 above expression requires 4 multiplies to get the result which
#                 can be reduced to 3 as we can find x_H*y_L + x_L*y_H using a 
#                 hack:
#                 x_H*y_L + x_L*y_H = (x_H + x_L)*(y_H + y_L) - x_H*y_H - x_L*y_L
#                 and we have already found x_H*y_H and x_L*y_L. so we are done
#                 in 3 multiply operations instead of 4.
